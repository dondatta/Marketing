...
class Sms_manager{
    ...
    public function set_credentioal($id) /**$api_id is for clickatell gateway***/
    {
	...
	****/
	...
	{	
		try
		{
			...
			$post_fields=array("email"=>$user,"password"=>$password,"device"=>$device_id,"number"=>$to,"message"=>$msg);
			$ch=curl_init($url);
			curl_setopt($ch, CURLOPT_POST, true);
			curl_setopt($ch, CURLOPT_POSTFIELDS, $post_fields);    
			curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);  
			...
			if($response['success'])
				$message_info['status']=$response['result']['success'][0]['status'];
				
			else
				$message_info['status']="Failed";
			
			
			$message_info['id']	=isset($response['result']['success'][0]['id']) ? $response['result']['success'][0]['id']:"";
