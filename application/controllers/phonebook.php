...
class phonebook extends Home
{
    ...
    public function add_contact_action()
    {
    	...
    	if ($_POST) 
    	{
            $this->form_validation->set_rules('first_name',$this->lang->line('First Name'),'trim|required');
            $this->form_validation->set_rules('last_name',$this->lang->line("Last Name"),'trim');
            $this->form_validation->set_rules('phone_number',$this->lang->line("Mobile Number"),'trim|required');
            ...
            if ($this->form_validation->run() == false || (empty($_POST['contact_type_id']))) 
            {
                 if(empty($_POST['contact_type_id']))                
       ...
       if ($_POST) 
        {
            ...
            if ($this->form_validation->run() == false || (!isset($_POST['contact_type_id']))) 
            {
    ...
    public function delete_contact_action($id=0)
    {
        if($id==0) exit(); 
        if($this->basic->update_data('contacts',array("id"=>$id,"user_id"=>$this->user_id),array("deleted"=>"1")))
        {
        ...
        if ($this->upload->do_upload('csv_file') != true) {
            ...
            if (!file_exists($csv) || !is_readable($csv)) {
                ...
                foreach ($data as $row) 
                {
                    ...
                    $insert_data['first_name'] = $row[0];
                    $insert_data['last_name'] = $row[1];
                    $insert_data['phone_number'] = $row[2];
                    $insert_data['email'] = $row[3];
                    ...
                    $insert_data['contact_type_id'] = str_replace('+',',',$row[5]);
     ...
     public function url_with_email_wise_download()
    {
        ...
        $head=array("First Name","Last Name","Mobile No.","Email","Date of Birth","Contact Group");
        fputcsv($fp, $head);
        $write_info = array();
        ...
        foreach ($info as  $value) 
        {
            ...
            $write_info[] = $value['contact_type_id'];          
